...
const GOOGLE_CLIENT_ID = "459844148501-9jqtil56lhpc56g2ddh6ol05jrgc3atu.apps.googleusercontent.com"; 
const GOOGLE_API_KEY = "AIzaSyAlNDPa0a4DKEWErU98IgZ61DJgpa3u9Z8"; 
const ROOT_FOLDER_ID = "1aY3zA-D3_tAhEFLmasuTCz3JURKeviKP"; 
const SENT_FOLDER_ID = "16_ie96ihd4lJouI8BcrqjjfQS_Rq7Ep0"; 
const DB_FOLDER_ID = "13L8CT9j-_Y-_sT6Xp6CEqQ52E0eZaB_Y"; 
  ...
  loadFromDrive: async (force = false) => {
    ...
    try {
        ...
        if (fileId) {
            ...
            if (data) {
                MOCK_FOLDERS = (data.folders || []).map((f: any) => ({ ...f, status: normalizeStatus(f.status) }));
  ...
  run: async (fname: string, ...args: any[]) => {
    ...
    if (GAS.token && fname === 'creaCartella') {
        ...
        try {
            ...
            MOCK_FOLDERS.unshift({ id: id, name: name, created: new Date().toLocaleDateString(), status: 'APERTA', fee: 0, paid: 0, lastUpdate: new Date().toLocaleDateString() });
            MOCK_DETAILS[id] = { 
                status: 'APERTA', 
                fee: '', 
                eredi: erediList,
    ...
    return new Promise((resolve) => {
        setTimeout(async () => {
          if (fname === 'getExistingSuccessionFolders') {
             ...
             const det = MOCK_DETAILS[id] || { status: 'APERTA', fee: '0', history: [] };
             det.status = normalizeStatus(det.status);
             resolve(det);
          } else if (fname === 'addLogToPractice') {
             const [id, text, type] = args;
             if (!MOCK_DETAILS[id]) MOCK_DETAILS[id] = { status: 'APERTA', fee:'0', history: [] };
             ...
             if (!MOCK_DETAILS[id]) MOCK_DETAILS[id] = { status: normalized, fee:'0', history: [] };
             ...
             const name = args[0];
             const erediList = args[1] || [];
             MOCK_FOLDERS.unshift({ id: newId, name: name, created: new Date().toLocaleDateString(), status: 'APERTA', fee: 0, paid: 0, lastUpdate: new Date().toLocaleDateString() });
             MOCK_DETAILS[newId] = { 
                status: 'APERTA', 
                fee: '', 
                eredi: erediList,
...
const SuccessioPro = () => {
  ...
  const filteredPractices = useMemo(() => {
      return archiveFolders.filter(f => {
          ...
          if (dashboardFilter === 'TUTTE') return true;
          if (dashboardFilter === 'DA INCASSARE') return (f.fee || 0) > (f.paid || 0);
          return normalizedF === dashboardFilter;
      });
  }, [archiveFolders, dashboardFilter]);
  ...
  const handleBatchUpload = async () => {
    ...
    try {
        for (let i = 0; i < tempFiles.length; i++) {
            ...
            const customName = tempNames[i] || 'DOC';
  ...
  const handleSubmitNewPractice = async () => {
    ...
    try {
      const folderId = await GAS.run('creaCartella', nomeCognome, eredi.map(e=>({id:e.id, nome:e.nome, parentela:e.parentela})));
         ...
         <div className="animate-in fade-in">
            <div style={{textAlign:'center', marginBottom:12}}>
             ...
             <div className={step !== 2 ? 'hidden' : ''}>
                 <label>Dichiarante:</label><select value={dichiaranteParentela} onChange={e=>setDichiaranteParentela(e.target.value)}><option value="">Grado parentela...</option><option value="Coniuge">Coniuge</option><option value="Figlio/a">Figlio/a</option><option value="Altro">Altro</option></select>
             ...
             <div className={step !== 3 ? 'hidden' : ''}>
                 ...
                 {eredi.map(erede => (
                    <div key={erede.id} className="erede-section animate-in">
                        <div style={{display:'flex', gap:8}}><select value={erede.parentela} onChange={e=>handleEredeChange(erede.id,'parentela',e.target.value)}><option value="">Grado...</option><option value="Coniuge">Coniuge</option><option value="Figlio/a">Figlio/a</option><option value="Altro">Altro</option></select><input type="text" value={erede.nome} onChange={e=>handleEredeChange(erede.id,'nome',e.target.value)} placeholder="Nome" /></div>
           ...
           {!selectedFolderId ? (
              <div>
                 ...
                 <div style={{overflowX:'auto'}}><table className="dashboard-table">
                    <thead><tr><th>Cliente</th><th>Stato</th><th>Parcella (€)</th><th>Incassato (€)</th><th>Progresso</th><th>Azioni</th></tr></thead>
